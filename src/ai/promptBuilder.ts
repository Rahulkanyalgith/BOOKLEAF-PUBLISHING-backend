...
import { retrieveRelevantContext } from './knowledgeBase';
...
export interface DraftResponsePromptParams {
  ...
  authorName: string;
  bookTitle?: string;
  previousMessages?: Array<{ sender: string; message: string; createdAt: string }>;
}

export const buildClassificationPrompt = (params: ClassificationPromptParams): string => {
...
export const buildPriorityPrompt = (params: PriorityPromptParams): string => {
...
export const buildDraftResponsePrompt = (params: DraftResponsePromptParams): string => {
  const context = retrieveRelevantContext(`${params.subject} ${params.description}`);
