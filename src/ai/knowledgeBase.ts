...
export const knowledgeBase: Record<string, KnowledgeSection> = {
  royalty: {
 ...
 */
export const retrieveRelevantContext = (text: string): string => {
  const normalizedText = text.toLowerCase();
  const matchedSections: string[] = [];

  for (const [, section] of Object.entries(knowledgeBase)) {
  ...
  if (matchedSections.length === 0) {
    matchedSections.push(knowledgeBase.general.context);
  }
